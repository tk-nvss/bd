  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-black tracking-tight text-[var(--foreground)] uppercase">Transactions</h2>
          <p className="text-[10px] text-[var(--muted)] font-medium mt-0.5">
            Monitor all customer transactions and payment history.
          </p>
        </div>

        <div className="flex items-center gap-3 shrink-0">
          <div className="px-2 py-1 rounded-lg bg-[var(--foreground)]/[0.03] border border-[var(--border)] hidden sm:flex items-center gap-2">
                  ...
                  {transactions.map((t, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span className="font-mono text-[9px] text-[var(--accent)] uppercase">{t.orderId}</span>
                        </td>
                        <td className="px-6 py-4">
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      <div className="space-y-1">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg border text-[8px] font-bold uppercase tracking-widest ${meta.class}`}>
                          {meta.icon}
                          {meta.label}
                        </span>
                    ...
                    <div className="flex items-center justify-between text-[11px] pt-3 border-t border-[var(--border)]">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-7 h-7 rounded-lg bg-[var(--foreground)]/[0.05] flex items-center justify-center shrink-0">
                          <Gamepad2 size={14} className="text-[var(--accent)]" />
                        </div>
                        <div className="truncate">
                          <p className="font-bold text-[var(--foreground)] text-[10px] uppercase leading-tight truncate">{t.gameSlug}</p>
                          <p className="text-[var(--muted)]/60 text-[10px] truncate leading-tight">{t.email || "No Email"}</p>
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-[10px] font-medium text-[var(--muted)]/40 tabular-nums">{new Date(t.createdAt).toLocaleDateString()}</p>
                      </div>
                    </div>
                  </motion.div>
                );
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-10">
                <DrawerSection icon={<Gamepad2 size={16} />} title="Game Details">
                  ...
                  <DrawerDetail label="Slug" value={selectedTx.itemSlug} />
                </DrawerSection>

                <DrawerSection icon={<Smartphone size={16} />} title="Player Details">
                  <DrawerDetail label="User ID" value={selectedTx.playerId} emphasize />
                  <DrawerDetail label="Zone" value={selectedTx.zoneId || "GLOBAL"} />
                </DrawerSection>
